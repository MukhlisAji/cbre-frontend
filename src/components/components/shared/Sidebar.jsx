...
} from "../../components/lib/const/Navigation";
import WarningModal from "./WarningModal";

const linkClasses =
  "flex items-center gap-2 px-2.5 hover:bg-c-weldon-blue hover:no-underline hover:text-white active:bg-c-teal rounded-sm text-sm";
...
export default function Sidebar() {
  ...
  return (
    <div className="flex relative">
      <MdKeyboardArrowLeft
        className={`bg-white z-20 text-c-teal text-2xl rounded-full absolute -right-3 top-2  border-c-dark-grayish cursor-pointer ${
          !isOpen && "rotate-180"
        }`}
        onClick={toggleSidebar}
      />
      <div
        className={`bg-white p-3 h-screen overflow-y-auto text-neutral-700 shadow-xl z-10 ${
          isOpen ? "w-64" : "w-16"
        } duration-300 relative flex flex-col justify-between`}
      >
        <div>
